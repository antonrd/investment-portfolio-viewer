class ShowStockBreakdowns {
  ...
  _showTotalBreakdown(all_breakdown) {
    ...
    $('.stock-breakdowns-all').append("<tr><th>Total value</th><th>Invested</th><th>Unrealized P/L</th><th>Unrealized P/L %</th><th>Realized P/L</th><th>Realized P/L %</th><th>Weighted cost</th><th>Weighted P/L %</th><th>Dividends</th><th>Gain</th><th>Total return %</th></tr>")
    $('.stock-breakdowns-all').append(this._totalStats(all_breakdown));
  }

  _totalStats(all_breakdown) {
    const total_gain = all_breakdown.unrealized_pl + all_breakdown.realized_pl + all_breakdown.total_dividends;
    const total_return_perc = all_breakdown.total_cost_basis > 0
      ? total_gain * 100 / all_breakdown.total_cost_basis : 0;
    return `<tr><td>$${MathHelpers.round2(all_breakdown.total_value)}
      </td><td>$${MathHelpers.round2(all_breakdown.total_invested)}
      ...
      </td><td>$${MathHelpers.round2(total_gain)}
      </td><td>${MathHelpers.round2(total_return_perc)}%
      </td></tr>`;
  }
