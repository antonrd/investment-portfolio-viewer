var ComputeSummaries = {
  ...
  init: function(stocks, trades, dividends) {
    ...
    this.stock_sells = {};
    this.stock_summary = {};
    this.stock_dividends = {};

    this.stock_details = {};
    this.dividends_details = {};
    ...
    for (var i = 0; i < stocks.length; i++) {
      ...
      this.stock_dividends[stock_symbol] = [];
    }

    for (var i = 0; i < dividends.length; i++){
      var dividend = dividends[i];
      this.dividends_details[dividend.payment_date] = dividend;
      if ( dividend.symbol in this.stock_dividends) {
        this.stock_dividends[dividend.symbol].push(this.dividends_details[dividend.payment_date]);
      }
    }
    ...
    for (var i = 0; i < trades.length; i++) {
      ...
      if (trade.symbol in this.stock_dividends){
        this.stock_dividends[trade.symbol].push(trade);
      }
    }

    for (var symbol in this.stock_dividends){
      this.stock_dividends[symbol].sort(this.sortEvents);
      var stock_amount = 0;
      var dividends_total = 0;
      var event = this.stock_dividends[symbol];
      for (var i = 0; i < this.stock_dividends[symbol].length; i++){
        if (event[i].hasOwnProperty('action')){
          if (event[i].action == Constants.BUY) {
            stock_amount += event[i].amount;
          } else if (event[i].action == Constants.SELL){
            stock_amount = stock_amount - event[i].amount;
          }
        } else {
          if (event[i].hasOwnProperty('dividend_amount')){
            dividends_total += stock_amount * event[i].dividend_amount;
          }
        }
        days_passed = (this.today - this.stock_dividends[symbol][0].date) / (1000 * 60 * 60 * 24);
        years = days_passed / 365;
        average_return = dividends_total / years;
      }
      this.stock_dividends[symbol] = {dividends_total: dividends_total};
      this.stock_dividends[symbol].average_return = average_return;
    }

    // console.log(this.stock_summary);

    console.log(this.stock_dividends);
    ...
    for (var symbol in this.stock_details) {
      ...
      this.stock_summary[symbol] = {
        ...
        dividends_total: this.stock_dividends[symbol].dividends_total,
        average_return: this.stock_dividends[symbol].average_return
      }
