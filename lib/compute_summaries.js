class ComputeSummaries {
  ...
  _computeSummaries(){
    ...
    for (var symbol in this.stock_details) {
      var buy_price = this._getStockBuyPrice(symbol);
      var buy_amount = this._getStockBuyAmount(symbol);
      var buy_price_days = this._getStockBuyPriceDays(symbol);
      var sell_price = this._getStockSellPrice(symbol);
      var sell_amount = this._getStockSellAmount(symbol);
      var sell_price_days = this._getStockSellPriceDays(symbol);
      if (buy_price == 0) {
        continue;
      }
  ...
  _collectStockEvents() {
    for (var i = 0; i < this.dividends.length; i++){
      var dividend = this.dividends[i];
  ...
  _computeDividendStats() {
    for (var symbol in this.stock_events){
      ...
      var average_return = this._computeAverageReturn(today, buy_events, dividends_total);
