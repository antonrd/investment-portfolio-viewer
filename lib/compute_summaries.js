var ComputeSummaries = {
  ...
  init: function(stocks, trades, dividends) {
    ...
    this.collectStockDetails(stocks);
    this.collectStockEvents(dividends, trades);
  ...
  collectStockEvents: function(dividends, trades) {
  ...
  collectStockDetails: function(stocks) {
  ...
  computeDividendStats: function() {
    for (var symbol in this.stock_events){
      ...
      var average_return = this.computeAverageReturn(today, buy_events, dividends_total);
      this.stock_events[symbol] = {dividends_total: dividends_total};
      this.stock_events[symbol].average_return = average_return;
    }
  },

  computeAverageReturn: function(today, buy_events, dividends_total) {
    var average_return = 0
    days_passed = (today - buy_events[0].date) / (1000 * 60 * 60 * 24);
    years = days_passed / 365;
    if (years < 1 || dividends_total == 0) {
      return average_return = 'N/A';
    } else {
      return average_return = dividends_total / years;
    }
  }
};
