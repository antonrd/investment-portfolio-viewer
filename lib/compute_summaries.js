class ComputeSummaries {
  ...
  _computeDividendStats() {
    for (let [key, value] of this.stock_events){
      ...
      let first_buy = events.find(this.getFirstBuy);
      let average_return = this._computeAverageReturn(today, first_buy.date, dividends_total);
      value.dividends_total = dividends_total;
      value.average_return = average_return;
    }
  }

  _computeAverageReturn(today, first_buy_date, dividends_total) {
