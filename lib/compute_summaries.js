class ComputeSummaries {

  constructor(stocks, trades, dividends) {
    this.stocks = stocks;
    this.trades = trades;
    this.dividends = dividends;
    this.total_portfolio_value = 0.0;
    this.today = new Date();

    this.stock_buys = new Map();
    this.stock_sells = new Map();
    this.stock_summary = new Map();
    this.stock_events = new Map();
    this.stock_details = new Map();
  }

  getStockSummaries() {
    this._collectStockDetails(this.stocks);
    this._collectStockEvents(this.dividends, this.trades);
    this._computeSummaries();
    return this.stock_summary;
  }

  _computeSummaries(){
    for (let trade of this.trades) {
      if (trade.action == Constants.BUY) {
        this._add_amount(trade.symbol, this.stock_buys, trade.amount, trade.price, trade.fees, trade.date);
      } else if (trade.action == Constants.SELL) {
        this._add_amount(trade.symbol, this.stock_sells, trade.amount, trade.price, -1 * trade.fees, trade.date);
      } else if (trade.action == Constants.SPLIT) {
        this._split_amount(trade.symbol, this.stock_buys, trade.amount)
        this._split_amount(trade.symbol, this.stock_sells, trade.amount)
      } else {
        console.error(`Unknown trade action: ${trade.action}`);
      }
    }

    this._computeDividendStats();

    for (let [key, value] of this.stock_details) {
      let buy_price = this._getStockBuyPrice(key) * Constants.CURRENCIES[value.currency];
      let buy_amount = this._getStockBuyAmount(key);
      let buy_price_days = this._getStockBuyPriceDays(key);
      let sell_price = this._getStockSellPrice(key) * Constants.CURRENCIES[value.currency];
      let sell_amount = this._getStockSellAmount(key);
      if (buy_price == 0) {
        continue;
      }

      let normalized_price = value.price * Constants.CURRENCIES[value.currency];

      let avg_buy_price = buy_price / buy_amount;
      let avg_sell_price = sell_price / sell_amount;
      // From the total amount spent on buying subtract the average cost for the shares already sold
      // to get the cost for purchasing the shares still left in the portfolio
      let left_shares_buy_cost = buy_price - sell_amount * avg_buy_price;
      let realized_pl = (avg_sell_price - avg_buy_price) * sell_amount;
      let realized_pl_perc = realized_pl * 100 / (sell_amount * avg_buy_price);
      let cost_of_sold_shares = sell_amount * avg_buy_price;

      let left_amount = buy_amount - sell_amount;
      let avg_left_buy_price = left_amount > 0 ? left_shares_buy_cost / left_amount : avg_buy_price;
      let unrealized_pl = left_amount * (normalized_price - avg_left_buy_price);
      let unrealized_pl_perc = left_shares_buy_cost > 0 ? unrealized_pl * 100 / left_shares_buy_cost : 0;

      let total_value = left_amount * normalized_price;
      let sell_amount_days = this._getStockSellAmountDays(key);
      let weighted_cost = (buy_price_days - sell_amount_days * avg_buy_price) / 365.0;
      let weighted_pl = unrealized_pl * 100 / weighted_cost;

      // Cost-weighted average holding period in years across all shares ever bought
      let avg_holding_years = buy_price > 0 ? (buy_price_days / buy_price) / 365.0 : 0;

      let value_date = value.price_date;
      let description = value.description;
      let currency = value.currency;
      let current_price = normalized_price;
      let dividends_total = value.dividends_total;
      let annual_yield = value.annual_yield;
      let current_amount = left_amount;

      this.stock_summary.set(key, {
        description,
        currency,
        current_price,
        buy_price,
        left_shares_buy_cost,
        avg_buy_price,
        avg_sell_price,
        realized_pl,
        realized_pl_perc,
        cost_of_sold_shares,
        unrealized_pl,
        unrealized_pl_perc,
        current_amount,
        total_value,
        value_date,
        weighted_cost,
        weighted_pl,
        dividends_total,
        annual_yield,
        avg_holding_years,
      });

      // console.log(key);
      this.total_portfolio_value += this.stock_summary.get(key).total_value;
      // console.log(this.total_portfolio_value);
    }

    for (let [key, value] of this.stock_summary) {
      // console.log(value.total_value);
      // console.log(this.total_portfolio_value);
      value.portion = value.total_value * 100 / this.total_portfolio_value;
    }
  }

  _add_amount(symbol, collection, amount, price, fees, date) {
    let days_passed = (this.today - date) / (1000 * 60 * 60 * 24);
    if (collection.has(symbol)) {
      collection.get(symbol).amount += amount;
      collection.get(symbol).price += amount * price + fees;
      collection.get(symbol).price_days += (amount * price + fees) * days_passed;
      collection.get(symbol).amount_days += amount * days_passed;
    } else {
      collection.set(symbol, {
        amount: amount,
        price: amount * price + fees,
        price_days: (amount * price + fees) * days_passed,
        amount_days: amount * days_passed,
      });
    }
  }

  _split_amount(symbol, collection, split_ratio) {
    if (!collection.has(symbol)) return;
    if (split_ratio > 0) {
      collection.get(symbol).amount *= split_ratio;
    } else {
      collection.get(symbol).amount /= -split_ratio;
    }
  }

  _getStockBuyAmount(symbol) {
    if (this.stock_buys.has(symbol)) {
      return this.stock_buys.get(symbol).amount;
    } else {
      return 0;
    }
  }

  _getStockBuyPrice(symbol) {
    if (this.stock_buys.has(symbol)) {
      return this.stock_buys.get(symbol).price;
    } else {
      return 0;
    }
  }

  _getStockBuyPriceDays(symbol) {
    if (this.stock_buys.has(symbol)) {
      return this.stock_buys.get(symbol).price_days;
    } else {
      return 0;
    }
  }

  _getStockSellAmount(symbol) {
    if (this.stock_sells.has(symbol)) {
      return this.stock_sells.get(symbol).amount;
    } else {
      return 0;
    }
  }

  _getStockSellPrice(symbol) {
    if (this.stock_sells.has(symbol)) {
      return this.stock_sells.get(symbol).price;
    } else {
      return 0;
    }
  }

  _getStockSellAmountDays(symbol) {
    if (this.stock_sells.has(symbol)) {
      return this.stock_sells.get(symbol).amount_days;
    } else {
      return 0;
    }
  }

  _sortEvents(a,b) {
    return a.date - b.date;
  }

  _collectStockEvents() {
    for (let dividend of this.dividends){
      if (this.stock_events.has(dividend.symbol)) {
        this.stock_events.get(dividend.symbol).push(dividend);
      } else {
        console.log(`${dividend.symbol} was not found in the portfolio!`);
      }
    }

    for (let trade of this.trades) {
      if (this.stock_events.has(trade.symbol)){
        this.stock_events.get(trade.symbol).push(trade);
      }
    }
  }

  _collectStockDetails() {
    for (let stock of this.stocks) {
      this.stock_details.set(stock.symbol, stock);
      this.stock_events.set(stock.symbol, []);
      // console.log(stock.symbol);
      // console.log(this.stock_details);
      // console.log(this.stock_details.get(stock.symbol));
    }
  }

  _computeDividendStats() {
    for (let [key, value] of this.stock_events){
      value.sort(this._sortEvents);
      let stock_amount = 0;
      let price_paid = 0.0;
      let dividends_total = 0.0;
      let total_yield = 0.0;
      let today = this.today;
      let events = value;
      let last_date_with_shares = today;

      for (let event of events){
        if (event.hasOwnProperty('action')){
          if (event.action == Constants.BUY) {
            stock_amount += event.amount;
            price_paid += event.price * event.amount;
            last_date_with_shares = today;
          } else if (event.action == Constants.SELL){
            let average_price_paid = price_paid / stock_amount;
            stock_amount -= event.amount;
            price_paid -= average_price_paid * event.amount;
            if (price_paid == 0) {
              last_date_with_shares = event.date;
            }
          } else if (event.action == Constants.SPLIT){
            if (event.amount > 0) {
              stock_amount *= event.amount;
            } else {
              stock_amount /= -event.amount;
            }
          }
        } else {
          if (event.hasOwnProperty('dividend_amount')){
            dividends_total += event.dividend_amount;
            total_yield += event.dividend_amount * 100 / price_paid;
          }
        }
      }

      this.stock_details.get(key).dividends_total = dividends_total;
      let first_buy = events.find(this._getFirstBuy);
      if (first_buy) {
        this.stock_details.get(key).annual_yield =
          this._computeAnnualYield(last_date_with_shares, first_buy.date, total_yield);
      } else {
        this.stock_details.get(key).annual_yield = 0.0
      }
    }
  }

  _computeAnnualYield(today, first_buy_date, total_yield) {
    let days_passed = (today - first_buy_date) / (1000 * 60 * 60 * 24);
    let years = days_passed / 365;
    if (years < 1 || total_yield == 0) {
      return 'N/A';
    } else {
      return total_yield / years;
    }
  }

  _getFirstBuy(event) {
    return event.action == "BUY";
  }
};
