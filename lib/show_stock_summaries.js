class ShowStockSummaries {
  ...
  show() {
    ...
    $('.stock-summaries').append(`<thead><tr><th>Symbol</th><th>Value ${Constants.DISPLAY_CURRENCY}</th><th>Amount</th><th>Date</th><th>Unrealized P/L</th><th>Unrealized P/L %</th><th>Realized P/L</th><th>Realized P/L %</th><th>Price</th><th>Average buy price</th><th>Average sell price</th><th>Dividends total</th><th>Yield</th><th>Portion</th></tr></thead>`);

    let nonsold_count = 0;
    let activeRows = '';
    for (let [key, value] of this.stock_summaries) {
      if (value.current_amount > 0) {
        activeRows += this._stock_row(key, value);
        ++nonsold_count;
      }
    }
    $('.stock-summaries').append(`<tbody id="active-rows">${activeRows}</tbody>`);
    ...
    $('.stock-summaries').append(`
      ...
      <tbody class="soldout-rows tablesorter-infoOnly" id="soldout-rows" style="display:none">${soldRows}</tbody>
    `);
    ...
    $('.stock-summaries').tablesorter({
      headers: {
        1: { sorter: 'digit' },  // Value
        2: { sorter: 'digit' },  // Amount
        3: { sorter: 'shortDate' }, // Date
        4: { sorter: 'digit' },  // Unrealized P/L
        5: { sorter: 'digit' },  // Unrealized P/L %
        6: { sorter: 'digit' },  // Realized P/L
        7: { sorter: 'digit' },  // Realized P/L %
        8: { sorter: 'digit' },  // Price
        9: { sorter: 'digit' },  // Avg buy price
        10: { sorter: 'digit' }, // Avg sell price
        11: { sorter: 'digit' }, // Dividends total
        12: { sorter: 'digit' }, // Yield
        13: { sorter: 'digit' }, // Portion
      }
    });
