class ComputeBreakdowns {
  ...
  _computeBreakdowns() {
    ...
    for (let [key, value] of this.stock_summaries) {
      ...
      this.total_cost_basis = (this.total_cost_basis || 0) + value.left_shares_buy_cost + value.cost_of_sold_shares;
      this.total_invested = (this.total_invested || 0) + value.left_shares_buy_cost;
    ...
    this.breakdown_totals.get(Constants.STOCK_ALL).total_cost_basis = this.total_cost_basis;
    this.breakdown_totals.get(Constants.STOCK_ALL).total_invested = this.total_invested;
  }
