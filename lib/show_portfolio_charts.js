class ShowPortfolioCharts {
  ...
  _buildBubbleDatasets(activeOnly) {
    ...
    for (let [symbol, s] of this.stockSummaries) {
      ...
      const totalReturn = s.unrealized_pl_perc + (s.realized_pl_perc || 0);
      ...
      datasets.push({
        ...
        _meta: { symbol, costBasis, active, unrealized_pl_perc: s.unrealized_pl_perc, realized_pl_perc: s.realized_pl_perc },
      });
    }
            ...
            callbacks: {
              label: (item) => {
                ...
                return [
                  ...
                  `Total return: ${item.raw.y}%`,
                  `  Unrealized: ${MathHelpers.round2(m.unrealized_pl_perc)}%`,
                  `  Realized: ${MathHelpers.round2(m.realized_pl_perc)}%`,
                  `Cost basis: ${Constants.DISPLAY_CURRENCY} ${cost}`,
                ];
              },
            },
          },
